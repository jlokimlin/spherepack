...
!
!         HALF*(b(0, n)*bbar(0, n, theta(i))+c(0, n)*cbar(0, n, theta(i)))
!
...
!
!               HALF*br(1, n+1)*vbar(0, n, theta(i))
!
!     plus the sum from m=1 to m=mmax-1 of the sum from n=m to 
!     n=nlat-1 of the real part of
!
...
!
!              -HALF*cr(1, n+1)*vbar(0, n, theta(i))
!
!     plus the sum from m=1 to m=mmax-1 of the sum from n=m to
!     n=nlat-1 of the real part of
!
...
contains
    ...
    pure function get_vhses_workspace_indices(ist, lnl, lzimn) result (return_value)
        !----------------------------------------------------------------------
        ! Dummy arguments
        !----------------------------------------------------------------------
    ...
    subroutine vhses_lower_routine(nlat, nlon, ityp, nt, imid, idvw, jdvw, v, w, mdab, &
        ...
        if (ityp <= 2) then
            ...
            do k=1, nt
                do j=1, nlon
                    v(1: imm1, j, k) = HALF*ve(1: imm1, j, k)
                    w(1: imm1, j, k) = HALF*we(1: imm1, j, k)
                end do
            end do
        end if
        ...
        if (mlat /= 0) then
            do k=1, nt
                v(imid, 1: nlon, k) = HALF*ve(imid, 1: nlon, k)
                w(imid, 1: nlon, k) = HALF*we(imid, 1: nlon, k)
            end do
        end if
