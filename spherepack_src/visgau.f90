...
subroutine VISGAU (nlat, nlon, h, len, eyer, eyelat, eyelon, &
                   theta, wk, lwk, iwk, liwk, ierror)
!
!     subroutine visgau produces a three dimensional visible rendering
!     of the function h(i, j) which is tabulated on a gauss distributed
!     colatitude grid.
!
!     requires  setgau alfk lfpt gaqd drst dintql dpytha 
!               visgau embed intrpg sptcg diag stride
!               trigau vsurf vsurf1 prjct box icvmg projct 
!
!     tvisgau uses the ncar graphics package.
...
!     input parameters
!
!     nlat   the number of gauss colatitudes.
!            if nlat is odd the equator is located at
...
!            center of the sphere to the surface at gauss colatitude 
!            point theta(i) and longitude point 
!            phi(j) = (j-1)*2*pi/nlon.
!
!     len    the first dimension of the array h as it appears in the
!            program that calls sphere.
!
...
10 continue
ierror = 0
!     ****     set up pointers to sub work arrays in wk
ntri = mn+mn
nw1 = 1
nw2 = nw1+mn
nclat = 1
...
!     either o or 1 depeending on whether the east, south, north
